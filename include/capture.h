...
/**
 * 从 AST 根节点构建 DFA，同时统计捕获组并存储子表达式元数据。
 *
 * 工作流程：
 *   1. 遍历 AST，为每个 AST_GROUP 分配捕获组编号（从 1 开始），收集子 AST 指针。
 *   2. 构建 NFA（Thompson 构造）→ DFA（子集构造法）。
 *   3. 将捕获组元数据（GroupMeta）存入全局映射表，key = DFA.states 指针。
 *   4. 匹配时通过 dfa_match_captured 在完整匹配区间内用子 DFA 定位各捕获组。
 *
